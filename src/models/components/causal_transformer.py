...
from src.models.utils.compile import no_compile
from src.models.components.gating import make_gating
from src.models.components.rope import RotaryEmbedding
...
class StreamingMultiheadAttention(nn.Module):
    ...
    def _complete_kv(self, k, v, state=None) -> KVCacheResult:
        if state is None:
            return KVCacheResult.from_kv(k, v)
        else:
            return state.kv_cache.complete(k, v)

    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, state=None):
        if state is None and self.causal:
            state = self._init_streaming_state(query.shape[0])
...
class StreamingTransformerLayer(nn.Module):
    ...
    def _init_streaming_state(self, batch_size: int) -> tp.Tuple[_LayerState, _MHAState]:
        layer_state = _LayerState(offset_cpu=0)
        if not self.skip_self_attn:
            mha_state = self.self_attn._init_streaming_state(batch_size)
        else:
            mha_state = None
        return layer_state, mha_state

    # feed forward block
    def _ff_block(self, x: torch.Tensor, layer_state=None) -> torch.Tensor:
        state = layer_state
        offset = 0
        if state is not None:
        ...
        ff_out = x_orig + self.layer_scale_2(update)
        return ff_out, state

    def _sa_block(self, x: torch.Tensor, mha_state=None):
        if self.skip_self_attn:
            return x
        x_orig = x
        x = self.norm1(x)
        update, mha_state = self.self_attn(x, x, x, mha_state)
        self_attention = x_orig + self.layer_scale_1(update)
        return self_attention, mha_state

    def forward(self, x: torch.Tensor, state=None):
        if state is None:
            state = self._init_streaming_state(x.shape[0])
        
        layer_state, mha_state = state

        with ExitStack() as stack:
            if x.device.type != 'cuda':
                stack.enter_context(no_compile())
            x, mha_state = self._sa_block(x, mha_state)
            x, layer_state = self._ff_block(x, layer_state)
            # state = self._streaming_state
            if layer_state:
                layer_state.offset_cpu += x.shape[1]

            state = (layer_state, mha_state)
            return x, state
...
class StreamingTransformer(nn.Module):
    ...
    def _init_streaming_state(self, batch_size: int): # -> _TransformerState:
        ...
        transformer_state = _TransformerState(offset=torch.zeros(1, device=device, dtype=torch.long))
        layer_class_state = [layer._init_streaming_state(batch_size) for layer in self.layers]
        return transformer_state, tuple(layer_class_state)
        # return _TransformerState(offset=torch.zeros(1, device=device, dtype=torch.long))

    def forward(self, x: torch.Tensor, state=None, *args, **kwargs):
        B, T, C = x.shape

        # state = self._streaming_state
        if state is None:
            state = self._init_streaming_state(B)
        transformer_state, layer_states = state

        if transformer_state is None:
            offset = torch.zeros(1, dtype=torch.long, device=x.device)
        else:
            offset = transformer_state.offset
        ...
        for layer, layer_state in zip(self.layers, layer_states):
            x, layer_state = layer(x, layer_state, *args, **kwargs)
            new_layer_states.append(layer_state)
        transformer_state.offset.add_(T)
        
        return x, (transformer_state, tuple(new_layer_states))
...
class ProjectedTransformer(nn.Module):
    ...
    def forward(self, x, state=None, *args, **kwargs):
        ...
        if state is None:
            state = self.transformer._init_streaming_state(x.shape[0])
        
        z, state = self.transformer(x, state, *args, **kwargs)
        ys = []
...
if __name__ == "__main__":
    ...
    causal = True
    context = 32
    transformer = StreamingTransformer(d_model, num_heads, num_layers, causal=causal, context=context)
    x = torch.randn(B, T, C)
    y, state = transformer(x)
    print(y.shape)
    print(state)
    print("Streaming transformer test passed")

    # Test projected transformer
    input_dimension = 512
    output_dimensions = (512, 512)
    d_model = 512
    projected_transformer = ProjectedTransformer(input_dimension, output_dimensions, d_model, num_heads=num_heads, num_layers=num_layers, causal=causal, context=context)
    x = torch.randn(B, T, input_dimension)
    y, state = projected_transformer(x)
    print(y[0].shape)
    print(y[1].shape)
    print(state)
    print("Projected transformer test passed")